<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(($mem1id!=$mem1det['ID']) or ($mem2id!=$mem2det['ID']) or ($mem3id!=$mem3det['ID'])){
            $resp['status']=-3;
        }else{
            ...
            $eveins = $con2->prepare("INSERT INTO events (event,captain,vicecaptain,3rdmem)  VALUES(?,?,?,?);");
            $eveins->bind_param("ssss",$evename,$m1,$m2,$m3);
            $evename = $event;
            $m1 = (string)$m1id;
            $m2 = (string)$m2id;
            $m3 = (string)$m3id;
            ...
            if($upd1->execute() and $upd2->execute() and $upd3->execute() and $eveins->execute()){
               $resp['status'] = 1;
