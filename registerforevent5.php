<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(($mem1id!=$mem1det['ID']) or ($mem2id!=$mem2det['ID'])){
            $resp['status']=-3;
        }else{
            ...
            $eveins = $con2->prepare("INSERT INTO events (event,captain,vicecaptain)  VALUES(?,?,?);");
            $eveins->bind_param("sss",$evename,$m1,$m2);
            $evename = $event;
            $m1 = (string)$m1id;
            $m2 = (string)$m2id;
            // $m3 = (string)$m3id;
            ...
            if($upd1->execute() and $upd2->execute() and $eveins->execute()){
               $resp['status'] = 1;
